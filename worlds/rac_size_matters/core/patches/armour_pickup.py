...
def prepare(pine, *, code_start, code, locations, checked=(), bypass_tier_gate=True):
    ...
    target(give + 0xB0)
    require(give + 0x130, packed(0xDFB00040, 0xDFB10048, 0xDFB20050,
                                0xDFB30058, 0xDFB40060, 0xDFBF0068,
    ...
    prefix = [m.lui(m.V0, (table + 0x8000) >> 16), m.addu(m.V0, m.V0, m.A0)]
    arena = bytes(flags) + packed(*prefix, m.lbu(m.V0, table & 0xFFFF, m.V0),
