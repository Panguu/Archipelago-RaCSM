...
class WeaponCycleState(IntEnum):
    IDLE   = 0x00  # assumed default — PS2 value, unconfirmed on PSP
    PICKUP = 0x0C  # set while a weapon pickup animation is playing — PS2 value, unconfirmed on PSP
...
class WeaponCyclerField:
    """Psp-backed accessor for one 4-byte weapon-cycler field. `attr` names
    ...
    def __get__(self, instance, owner) -> int | None:
        ...
        if address is None:
            return None
        return instance.psp.read_int32(address)

    def __set__(self, instance, value: int) -> None:
        address = getattr(instance, self.attr)
        if address is not None:
            instance.psp.write_int32(address, value)


class WeaponCyclerInventory:
    """Psp-backed live accessor + write gate for the per-planet weapon
    ...
    set_base(planet_id). Ported from PS2 — no PSP addresses are known yet,
    so every field reads as None and every write is a no-op until
    PlanetAddresses.weapon_cycler_* is filled in per planet.
    ...
    def __init__(self, psp: Psp) -> None:
        self.psp = psp
