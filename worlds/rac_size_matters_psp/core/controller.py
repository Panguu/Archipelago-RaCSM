...
class ControllerButtons(IntFlag):
    # CONFIRMED live on PSP (bit values match PS2 exactly). PSP has no L2/R2
    # (single shoulder pair, no second analog stick) — those bits have no
    # physical button and will never read as pressed.
    L1       = 0x01
    R1       = 0x02
    TRIANGLE = 0x10
    CIRCLE   = 0x20
...
class GlobalButtonState:
    ...
    so a bit reads 0 when pressed and 1 when released, hence the
    ``~x & 0xFF`` inversion below.
    """
    ...
    @classmethod
    def read(cls, ipc: Psp, planet_id: int) -> "GlobalButtonState":
        ...
        # PSP addresses in the table are stored as real, direct addresses
        # (unlike PS2's short-form + 0x20000000 EE-RAM-mirror OR trick, which
        # has no PSP equivalent and has been dropped). buttons byte is always
        # pause_select_addr + 1.
        return cls(
            ipc.read_int8(pause_select_addr),
            ipc.read_int8(pause_select_addr + 1),
        )
