...
class Core:
    ...
    def apply_inventory(
        ...
        *,
        ...
        Called every tick during normal gameplay (see PspMixin._poll_game())
    ...
    def notify(self, text: bytes | str) -> None:
        """In-game text-box notifications aren't supported on PSP (no
        SmallTextBox/MultiLineTextBox address support — see
        core/display_text.py's removal). Log-only stand-in so every existing
        caller (item received, deathlink, connection status, ...) keeps
        working without each needing its own change. Callers often still
        build messages with display_text.colored_text(), which embeds raw
        0x90-prefixed colour-marker byte pairs and a trailing NUL — strip
        those out so the log line reads as plain text."""
    ...
    def tick(self) -> None:
        """One poll cycle. Called by the client's own poll loop (PspMixin)
        ...
        if became_ready:
            ...
            if self.clank_enabled:
                ...
                #
                # CLANK_SECTION_UNLOCK_ADDRESSES (and every clank/skyboard base
                # address) are now CONFIRMED live on PSP, but keep the guard
                # anyway so a future regression doesn't take the whole client
                # down; disable clank challenges for the rest of the session
                # on first failure instead of retrying every transition.
                try:
