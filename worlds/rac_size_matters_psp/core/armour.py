...
class ArmourPiece(IntFlag):
    ...
    - Bit 1 (0x02): Helmet
    - Bits 2-3 (0x0C): Gloves — CONFIRMED on PSP; differs from the PS2 build's
      single-bit 0x04. Live-verified: Wildfire chestplate+gloves owned read
      back as 0x0D (0x01 chestplate | 0x0C gloves).
    - Bit 4 (0x10): Boots (any value with bit 4 set is considered to have boots equipped) — not yet PSP-verified
    """
...
class EquippedArmour:
    ...
    @classmethod
    def from_ap_data(cls, base: int, pine: Psp, data: dict[str, int]) -> EquippedArmour:
...
class ArmourUnlocks:
    ...
    @classmethod
    def from_ap_data(cls, base: int, pine: Psp, data: dict[str, int]) -> ArmourUnlocks:
